#!/usr/bin/env python3
"""Validate web-fallback branch protection proof for main branch."""
...
def _render_text(result: Dict[str, Any]) -> str:
    ...
    lines = [f"{status}: remote branch-protection web proof validation"]
    lines.append(f"- proof_file: {result.get('proof_file', '<unknown>')}")
    if result.get("required_status_checks_present") is not None:
        lines.append("- required_status_checks_present: " + ", ".join(result.get("required_status_checks_present") or []))
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate REMOTE_PROOF_BRANCH_PROTECTION_WEB.json.")
    parser.add_argument("--proof-file", required=True, help="Path to REMOTE_PROOF_BRANCH_PROTECTION_WEB.json")
...
def main() -> int:
    ...
    try:
        result = remote_proof_contract.validate_branch_file(proof_file)
        status = result["status"]
        ...
        print(f"FAIL: remote branch web proof internal validation error: {exc}")
